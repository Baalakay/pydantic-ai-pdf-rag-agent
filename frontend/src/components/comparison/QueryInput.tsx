...
import type { QueryAnalysis } from '@/types/comparison';
...
export function QueryInput({ onQuery, disabled }: QueryInputProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // First, analyze the query to get the model names
      const queryAnalysis = await analyzeQuery(query);
      
      // Then use the model names from the query analysis for the combined analysis
      const result = await analyzeCombined(query, queryAnalysis.models, {});
      console.log('Combined analysis response:', result);

      if (!result || !result.query_analysis) {
        throw new Error('Invalid response from server');
      }

      // The query_analysis from the backend is already in the correct format
      setLoadingState('processing');
      onQuery(result.query_analysis);
    } catch (err) {
